 ...
 */
SuperMap.Feature.ShapeFactory = SuperMap.Class({
     ...
     * {Object} 返回图形工厂类对象。
     */
    initialize: function(shapeParameters) {
    ...
    createShape: function(shapeParameters) {
        ...
        if(sps instanceof SuperMap.Feature.ShapeParameters.Point){        // 点
            //设置style
            ...
            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x', 'y']);
            ...
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['x', 'y', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Line){        // 线
            //检查参数 pointList 是否存在
            ...
            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['pointList']);

            // 创建图形
            var shape = new SuperMap.LevelRenderer.Shape.SmicBrokenLine();
            shape.style = SuperMap.Feature.ShapeFactory.transformStyle(style);
            shape.highlightStyle = SuperMap.Feature.ShapeFactory.transformStyle(sps.highlightStyle);
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['pointList', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Polygon){        // 面
            //检查参数 pointList 是否存在
            if(!sps.pointList) return null;
        ...
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Rectangle){        // 矩形
            //检查参数 pointList 是否存在
            ...
            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x', 'y', 'width', 'height']);

            //创建图形
            var shape = new SuperMap.LevelRenderer.Shape.SmicRectangle();
            shape.style = SuperMap.Feature.ShapeFactory.transformStyle(style);
            shape.highlightStyle = SuperMap.Feature.ShapeFactory.transformStyle(sps.highlightStyle);
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['x', 'y', 'width', 'height', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Sector){        // 扇形
            //设置style
            var style = new Object();
            style["x"] = sps.x;
            style["y"] = sps.y;
            ...
            if(sps["r0"]){style["r0"] = sps.r0};
            if(sps["clockWise"]){style["clockWise"] = sps.clockWise};

            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x', 'y', 'r', 'startAngle', 'endAngle', 'r0', 'endAngle']);

            //创建图形
            var shape = new SuperMap.LevelRenderer.Shape.SmicSector();
            shape.style = SuperMap.Feature.ShapeFactory.transformStyle(style);
            shape.highlightStyle = SuperMap.Feature.ShapeFactory.transformStyle(sps.highlightStyle);
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['x', 'y', 'r', 'startAngle', 'endAngle', 'r0', 'endAngle', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Label){        // 标签
            //设置style
            var style = new Object();
            style["x"] = sps.x;
            style["y"] = sps.y;
            ...
            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x', 'y', 'text']);

            //创建图形
            var shape = new SuperMap.LevelRenderer.Shape.SmicText();
            shape.style = SuperMap.Feature.ShapeFactory.transformStyle(style);
            shape.highlightStyle = SuperMap.Feature.ShapeFactory.transformStyle(sps.highlightStyle);
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['x', 'y', 'text', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Image){        // 图片
            //设置style
            var style = new Object();
            style["x"] = sps.x;
            style["y"] = sps.y;
            if(sps["image"]){style["image"] = sps.image;}
            if(sps["width"]){style["width"] = sps.width;}
            if(sps["height"]){style["height"] = sps.height;}
            if(sps["sx"]){style["sx"] = sps.sx;}
            if(sps["sy"]){style["sy"] = sps.sy;}
            if(sps["sWidth"]){style["sWidth"] = sps.sWidth}
            if(sps["sHeight"]){style["sHeight"] = sps.sHeight}

            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x', 'y', 'image', 'width', 'height', 'sx', 'sy', 'sWidth', 'sHeight']);

            //创建图形
            var shape = new SuperMap.LevelRenderer.Shape.SmicImage();
            shape.style = SuperMap.Feature.ShapeFactory.transformStyle(style);
            shape.highlightStyle = SuperMap.Feature.ShapeFactory.transformStyle(sps.highlightStyle);
            SuperMap.Util.copyAttributesWithClip(shape, sps, ['x', 'y', 'image', 'width', 'height', 'style', 'highlightStyle']);

            return shape;
        }
        else if(sps instanceof SuperMap.Feature.ShapeParameters.Circle){       //圆形 用于符号专题图
            //设置stytle
            var style = new Object();
            style["x"] = sps.x;
            style["r"] = sps.r;
            ...
            style = SuperMap.Util.copyAttributesWithClip(style, sps.style, ['x','y','r']);
            ...
            SuperMap.Util.copyAttributesWithClip(shape,sps,['x','y','r','style','highlightStyle','lineWidth','text','textPosition']);
 ...
 */
SuperMap.Feature.ShapeFactory.transformStyle = function(style){
    var newStyle = {};
    ...
    for(var ss in style){
        switch(ss){
            case "fill":
                brushType[0] = style[ss]; break;
                break;
            case "fillColor":
                newStyle["color"] = style[ss]; break;
            case "stroke":
                brushType[1] = style[ss]; break;
                break;
            case "strokeWidth":
                newStyle["lineWidth"] = style[ss]; break;
            case "strokeLinecap":
                newStyle["lineCap"] = style[ss]; break;
            case "strokeLineJoin":
                newStyle["lineJoin"] = style[ss]; break;
            case "strokeDashstyle":
                newStyle["lineType"] = style[ss]; break;
            case "pointRadius":
                newStyle["r"] = style[ss]; break;
            case "label":
                newStyle["text"] = style[ss]; break;
            case "fontColor":
                newStyle["textColor"] = style[ss]; break;
            case "fontStyle":
                fontStr[0] = style[ss ]; break;
            case "fontVariant":
                fontStr[1] = style[ss]; break;
            case "fontWeight":
                fontStr[2] = style[ss]; break;
            case "fontSize":
                fontStr[3] = style[ss] + "px"; break;
            case "fontFamily":
                fontStr[4] = style[ss]; break;
            case "fontOpacity":
                newStyle["opacity"] = style[ss]; break;
            case "labelPosition":
                newStyle["textPosition"] = style[ss]; break;
            case "labelAlign":
                newStyle["textAlign"] = style[ss]; break;
            case "labelBaseline":
                newStyle["textBaseline"] = style[ss]; break;
            case "labelRotation":
                newStyle["textRotation"] = style[ss]; break;


            default:  newStyle[ss] = style[ss]; break;
        }
    }
...
SuperMap.Feature.ShapeFactory.Background = function(shapeFactory, box, setting){
    ...
    // 背景框图形参数对象
    var bgSP = new SuperMap.Feature.ShapeParameters.Rectangle(box[0], box[3], Math.abs(box[2] - box[0]), Math.abs(box[3] - box[1]));
    ...
    if(sets.backgroundStyle){
        SuperMap.Util.copyAttributesWithClip(bgSP.style, sets.backgroundStyle);
    }
...
SuperMap.Feature.ShapeFactory.GraphAxis = function(shapeFactory, dataViewBox, setting, xShapeInfo){
    ...
    if(axisytick == 0){
       ...
       xMainPois.push([dvb[0], dvb[1]]);
        ...
        for(var i = 0; i < axisytick; i++){
            ...
            if(isAddRefLine){
                // 参考线参数对象
                var refLineSP = new SuperMap.Feature.ShapeParameters.Line([
                    [dvb[0], thckY],
                    [dvb[2], thckY]
                ]);
                // 参考线默认样式对象
                ...
                if(sets.xReferenceLineStyle){
                    SuperMap.Util.copyAttributesWithClip(refLineSP.style, sets.xReferenceLineStyle);
                }
                // 生成参考线图形对象
    ...
    if(sets.axisUseArrow){
        ...
        //x轴箭头
        var xSP = new SuperMap.Feature.ShapeParameters.Polygon(xArrowPois);
        xSP.style = {fillColor: "#008acd"};
        SuperMap.Util.copyAttributesWithClip(xSP.style, sets.axisStyle);
        arrows.push(shapeFactory.createShape(xSP));

        //y轴箭头
        var ySP = new SuperMap.Feature.ShapeParameters.Polygon(yArrowPois);
        ySP.style = {fillColor: "#008acd"};
        SuperMap.Util.copyAttributesWithClip(ySP.style, sets.axisStyle);
        arrows.push(shapeFactory.createShape(ySP));

        // z轴箭头 坐标轴箭头是否要使用
        if(sets.axis3DParameter && !isNaN(sets.axis3DParameter) && sets.axis3DParameter >= 15){
            var zSP = new SuperMap.Feature.ShapeParameters.Polygon(zArrowPois);
            zSP.style = {fillColor: "#008acd"};
            SuperMap.Util.copyAttributesWithClip(zSP.style, sets.axisStyle);
    ...
    // 坐标轴参数对象
    var axisSP = new SuperMap.Feature.ShapeParameters.Line(pois);
    // 坐标轴默认style
    axisSP.style = {
        strokeLinecap: "butt",
        strokeLineJoin: "round",
    ...
    // 用户 style
    if(sets.axisStyle){
        SuperMap.Util.copyAttributesWithClip(axisSP.style, sets.axisStyle);
    }
    // 禁止事件
    ...
    if(sets.axisYLabels && sets.axisYLabels.length && sets.axisYLabels.length > 0){
        ...
        if(len == 1){
            // 标签参数对象
            var labelYSP = new SuperMap.Feature.ShapeParameters.Label(dvb[0] - 5 + ylOffset[0], dvb[3] + ylOffset[1], axisYLabels[0]);
            labelYSP.style = {
                labelAlign: "right"
            };
            // 用户 style
            if(sets.axisYLabelsStyle)    {
                SuperMap.Util.copyAttributesWithClip(labelYSP.style, sets.axisYLabelsStyle);
            }
            // 禁止事件
            ...
            for(var j = 0; j < len; j++){
                // 标签参数对象
                var labelYSP = new SuperMap.Feature.ShapeParameters.Label(dvb[0] - 5 + ylOffset[0], labelY + ylOffset[1], axisYLabels[j]);
                labelYSP.style = {
                    labelAlign: "right"
                };
                // 用户 style
    ...
    if(sets.axisXLabels && sets.axisXLabels.length && sets.axisXLabels.length > 0){
        ...
        if(xShapeInfo && xShapeInfo.xPositions && xShapeInfo.xPositions.length && xShapeInfo.xPositions.length == len){
            ...
            for(var K = 0; K < len; K++){
                // 标签参数对象
                var labelXSP = new SuperMap.Feature.ShapeParameters.Label(xsCenter[K] + xlOffset[0], dvb[1] + xlOffset[1], axisXLabels[K]);
                // 默认 style
                labelXSP.style = {
                    labelAlign: "center",
                    labelBaseline: "top"
                ...
                if(sets.axisXLabelsStyle){
                    SuperMap.Util.copyAttributesWithClip(labelXSP.style, sets.axisXLabelsStyle);
                }
                // 禁止事件
            ...
            if(len == 1){
                // 标签参数对象
                var labelXSP = new SuperMap.Feature.ShapeParameters.Label(dvb[0] - 5 + xlOffset[0], dvb[1] + xlOffset[0], axisXLabels[0]);
                // 默认 style
                labelXSP.style = {
                    labelAlign: "center",
                    labelBaseline: "top"
                ...
                for(var m = 0; m < len; m++){
                    // 标签参数对象
                    var labelXSP = new SuperMap.Feature.ShapeParameters.Label(labelX + xlOffset[0], dvb[1] + xlOffset[1], axisXLabels[m]);
                    // 默认 style
                    labelXSP.style = {
                        labelAlign: "center",
                        labelBaseline: "top"
...
SuperMap.Feature.ShapeFactory.ShapeStyleTool = function(defaultStyle, style, styleGroup, styleByCodomain, index, value){
    ...
    // 基础 style
    if(style){
        SuperMap.Util.copyAttributesWithClip(finalStyle, style);
    }

    // 按索引赋 style
    if(styleGroup && styleGroup.length && typeof(index) !== "undefined" && !isNaN(index) && index >= 0){
        if(styleGroup[index]){
            SuperMap.Util.copyAttributesWithClip(finalStyle, styleGroup[index]);
        }
    }
    ...
    if(styleByCodomain && styleByCodomain.length && typeof(value) !== "undefined"){
        ...
        for(var i = 0; i < dscLen; i++){
            if(dsc[i].start <= v && v < dsc[i].end){
                SuperMap.Util.copyAttributesWithClip(finalStyle, dsc[i].style);
                break;
            }
        }
    }
