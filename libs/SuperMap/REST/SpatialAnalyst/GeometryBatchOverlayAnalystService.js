 ...
 * @requires SuperMap/REST/SpatialAnalyst/GeometryBatchOverlayAnalystParameters.js
 * @requires SuperMap/REST/SpatialAnalyst/GeometryBatchOverlayAnalystResult.js
...
/**
 * Class: SuperMap.REST.GeometryBatchOverlayAnalystService
 * 批量几何叠加分析服务类
 ...
 */
SuperMap.REST.GeometryBatchOverlayAnalystService = SuperMap.Class(SuperMap.ServiceBase, {
     ...
     * 例如:
     * (start code)     
     * var myOverlayAnalystService = new SuperMap.REST.GeometryBatchOverlayAnalystService(url);
     ...
     * {Object} 监听器对象，在构造函数中设置此参数（可选），对 GeometryBatchOverlayAnalystService 支持的两个事件 processCompleted 、processFailed 进行监听，相当于调用 SuperMap.Events.on(eventListeners)。
     */
    eventListeners: null,

    /** 
     * Property: lastResult
     * {{<SuperMap.REST.GeometryBatchOverlayAnalystResult>} 服务端返回的叠加分析结果数据。
     */
    lastResult: null,
    ...
    /**
     * Constructor: SuperMap.REST.GeometryBatchOverlayAnalystService
     * 查询叠加分析服务基类构造函数。
     *     
     * 例如：
     * (start code)     
     ...
     *
     * Parameters:
     * params - {<SuperMap.REST.GeometryBatchOverlayAnalystParameters>} 
     */
    processAsync: function(parameter) {
        ...
        me.url += 'geometry/overlay/batch';
        me.isInTheSameDomain = true;
        var parameterObject = SuperMap.REST.GeometryBatchOverlayAnalystParameters.toObject(parameter);
        var jsonParameters = SuperMap.Util.toJSON(parameterObject);

        if (me.isInTheSameDomain) {
            me.url += '.json?returnContent=true&ignoreAnalystParam=true';
        } else {
            me.url += '.jsonp?returnContent=true&ignoreAnalystParam=true';
        }
    ...
    analyzeComplete: function(result) {
        ...
        var analyzeResult = SuperMap.REST.GeometryBatchOverlayAnalystResult.fromJson(result);
    ...
    CLASS_NAME: "SuperMap.REST.GeometryBatchOverlayAnalystService"
});